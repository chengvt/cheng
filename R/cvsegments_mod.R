...
cvsegments_mod <- function(N, k, length.seg = ceiling(N / k),
    ...
    switch(type, 
           ...
           interleaved = {
               if (blindsize > 1){
                   length.seg = ceiling(N / (k * blindsize)) * blindsize
                   incomplete <- k * length.seg - N    # Number of incomplete segments
               }
               m <- c(1:N, rep(NA, incomplete))
               inds <- do.call(rbind, lapply(split(m, rep(seq_len(length(m) /(blindsize*k)), each = blindsize*k)), 
                                             matrix, nrow = blindsize))
